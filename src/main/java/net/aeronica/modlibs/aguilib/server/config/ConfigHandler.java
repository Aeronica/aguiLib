...
public enum ConfigHandler {
    ...
    private class ConfigContainer {
        ...
        private final List<net.aeronica.modlibs.aguilib.server.config.EntryProperty> entryProperties;
        ...
        public ConfigContainer(ModContainer mod, Object wrappedConfig, Configuration forgeConfiguration) {
            ...
            this.entryProperties = Arrays.stream(wrappedConfig.getClass().getFields())
                    ...
                    .map(field -> {
                        Class<? extends net.aeronica.modlibs.aguilib.server.config.EntryProperty> entryPropertyClass = net.aeronica.modlibs.aguilib.server.config.EntryProperty.getBuiltInPropertyClass(field.getType());
                        if (entryPropertyClass == null) {
                            entryPropertyClass = net.aeronica.modlibs.aguilib.server.config.ConfigHandler.this.entryPropertyClasses.get(field.getType());
                        }
                        if (entryPropertyClass != null) {
                            try {
                                Constructor<? extends net.aeronica.modlibs.aguilib.server.config.EntryProperty> constructor = entryPropertyClass.getConstructor(Object.class, Field.class, Configuration.class);
                                return constructor.newInstance(wrappedConfig, field, forgeConfiguration);
                            } catch (Exception e) {
                                AGuiLib.LOGGER.error("Failed to create property entry {} for mod {}", field.getName(), mod.getName(), e);
                            }
                        } else {
                            AGuiLib.LOGGER.error("Found unsupported config entry {} for mod {}", field.getName(), mod.getName());
                        }
                        return null;
                    })
        ...
        public void save() {
            this.entryProperties.forEach(net.aeronica.modlibs.aguilib.server.config.EntryProperty::save);
            this.forgeConfiguration.save();
        }

        public void load() {
            this.entryProperties.forEach(net.aeronica.modlibs.aguilib.server.config.EntryProperty::load);
        }

        public void reset() {
            this.entryProperties.forEach(net.aeronica.modlibs.aguilib.server.config.EntryProperty::reset);
        }
    }

    private Map<Class<?>, Class<net.aeronica.modlibs.aguilib.server.config.EntryProperty>> entryPropertyClasses = new HashMap<>();
     ...
     */
    public <T> void registerEntryPropertyClass(Class<T> type, Class<net.aeronica.modlibs.aguilib.server.config.EntryProperty> entryAdapter) {
    ...
    public void injectConfig(ModContainer mod, ASMDataTable data) {
        ...
        if (annotations != null) {
            Set<ASMDataTable.ASMData> targetList = annotations.get(net.aeronica.modlibs.aguilib.server.config.Config.class.getName());
            ...
            for (ASMDataTable.ASMData target : targetList) {
                try {
                    ...
                    AGuiLib.LOGGER.fatal("Failed to inject config for mod container {}", e, mod);
                }
            }
        }
    }
