...
public class ButtonElement<T extends IElementGUI> extends Element<T> {
    public static final ColorScheme CLOSE = ColorScheme.create(() -> AGuiLib.CONFIG.getDarkAccentColor(), () -> 0xFFE04747);
    ...
    public void render(float mouseX, float mouseY, float partialTicks) {
        ...
        fontRenderer.drawString(this.text, this.getPosX() + (this.getWidth() / 2) - (fontRenderer.getStringWidth(this.text) / 2), this.getPosY() + (this.getHeight() / 2) - (fontRenderer.FONT_HEIGHT / 2), AGuiLib.CONFIG.getTextColor(), false);
    }
