...
public abstract class InputElementBase<T extends IElementGUI> extends Element<T> {
    ...
    public void render(float mouseX, float mouseY, float partialTicks) {
        this.drawRectangle(this.getPosX(), this.getPosY(), this.getWidth(), this.getHeight(), this.editable ? AGuiLib.CONFIG.getSecondaryColor() : AGuiLib.CONFIG.getSecondarySubcolor());
        ...
        if (!displayString.isEmpty()) {
            ...
            line = fontRenderer.drawString(s, x + 3, y + 1 + this.getHeight() / 2 - fontRenderer.FONT_HEIGHT / 2, AGuiLib.CONFIG.getTextColor(), false);
        }
        ...
        if (!displayString.isEmpty() && verticalCursor && cursor < displayString.length()) {
            fontRenderer.drawString(displayString.substring(cursor), line + 1, y + 1 + this.getHeight() / 2 - fontRenderer.FONT_HEIGHT / 2, AGuiLib.CONFIG.getTextColor(), false);
        }

        if (renderCursor) {
            if (renderVerticalCursor) {
                this.drawRectangle(lineX, y, 1, this.getHeight() / 2 - fontRenderer.FONT_HEIGHT / 2 + 1 + fontRenderer.FONT_HEIGHT, AGuiLib.CONFIG.getPrimaryColor());
            } else {
                fontRenderer.drawString("_", cursor == 0 ? lineX + 3 : lineX, y + 1 + this.getHeight() / 2 - fontRenderer.FONT_HEIGHT / 2, AGuiLib.CONFIG.getPrimaryColor(), false);
            }
        }
