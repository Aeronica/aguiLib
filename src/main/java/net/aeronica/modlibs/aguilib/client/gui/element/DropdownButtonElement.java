...
public class DropdownButtonElement<T extends IElementGUI> extends Element<T> {
    ...
    public void render(float mouseX, float mouseY, float partialTicks) {
        ...
        fontRenderer.drawString(text, this.getPosX() + (this.getWidth() / 2) - (fontRenderer.getStringWidth(text) / 2), this.getPosY() + (this.getHeight() / 2) - (fontRenderer.FONT_HEIGHT / 2), AGuiLib.CONFIG.getTextColor(), false);
        if (this.dropped) {
            ...
            for (String entry : this.selected.getValidStringValues()) {
                ...
                fontRenderer.drawString(entry, this.getPosX() + 3, y, AGuiLib.CONFIG.getTextColor(), false);
                y += 12.0F;
            }
        }
    }
