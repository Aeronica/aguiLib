    ...
    public void render(float mouseX, float mouseY, float partialTicks) {
        ...
        for (String entry : this.entries) {
            ...
            if (this.selectedEntry == entryIndex) {
                this.drawRectangle(entryX, entryY, entryWidth, this.entryHeight, clickSelecting ? AGuiLib.CONFIG.getAccentColor() : AGuiLib.CONFIG.getDarkAccentColor());
            } else {
                this.drawRectangle(entryX, entryY, entryWidth, this.entryHeight, selected ? AGuiLib.CONFIG.getAccentColor() : this.getColorScheme().getSecondaryColor());
            }
            fontRenderer.drawString(entry, entryX + 2, (entryY - fontRenderer.FONT_HEIGHT / 2) + (this.entryHeight / 2), AGuiLib.CONFIG.getTextColor(), false);
            y += this.entryHeight;
            entryIndex++;
        }
